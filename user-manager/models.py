...
class User(db.Model):
    ...
    _iban = db.Column('iban', db.String(500), nullable=True)  # Encrypted, we need increased length
    data_registrazione = db.Column(db.DateTime, default=datetime.now(timezone.utc))
    request_id = db.Column(db.String(255), unique=True) # Unique request ID for idempotency
